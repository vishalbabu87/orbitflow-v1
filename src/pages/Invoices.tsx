...
import { Plus, FileText, Search, Filter, X, Calendar, Download } from 'lucide-react';
import { cn } from '@/lib/utils';

// Mock Data
const invoices = Array.from({ length: 12 }).map((_, i) => ({
  id: `INV-2026-${101 + i}`,
  client: `Client ${i + 1}`,
  email: `billing${i}@client.com`,
  amount: (Math.random() * 8000 + 500).toFixed(2),
  status: i % 3 === 0 ? 'Paid' : i % 4 === 0 ? 'Overdue' : i % 5 === 0 ? 'Draft' : 'Sent',
  date: `Oct ${24 - i}, 2026`,
}));
...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case 'Draft':
    default:
      return 'bg-gray-100 text-[var(--orbit-text-secondary)] border-gray-200 dark:border-gray-800 dark:bg-gray-850/60 dark:text-gray-400';
  }
};
...
export default function Invoices() {
  ...
  const filteredInvoices = invoices.filter((invoice) => {
    ...
    const matchesStatus = statusFilter ? `status_${invoice.status.toLowerCase()}` === statusFilter : true;
  ...
  return (
    <div className="animate-in fade-in relative mx-auto w-full max-w-7xl flex-1 p-4 text-[var(--orbit-text-primary)] duration-500 md:p-8">
      ...
      <div className="mb-4 flex flex-col justify-between gap-3 md:mb-6 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed font-normal text-[var(--orbit-text-secondary)] sm:text-sm">
            Manage billing, receipts, and payment status.
          </p>
        </div>
        <div className="grid w-full grid-cols-1 items-center gap-2 sm:flex sm:w-auto">
          <button
            type="button"
      ...
      <div className="w-full overflow-hidden rounded-xl border border-[var(--orbit-border-subtle)] bg-[var(--orbit-surface)] md:rounded-2xl dark:border-gray-800 dark:bg-gray-900">
        ...
        <table className="w-full min-w-[800px] text-left text-xs whitespace-nowrap">
            <thead className="border-b border-[var(--orbit-border-subtle)] bg-[var(--orbit-elevated)] font-mono text-xs font-bold tracking-widest text-[var(--orbit-text-muted)] uppercase sm:text-xs dark:border-gray-800 dark:bg-gray-800/50 dark:text-[var(--orbit-text-secondary)]">
              <tr>
                <th className="px-4 py-3 sm:px-6 sm:py-4">Invoice ID</th>
                ...
                >
                  <td className="px-4 py-2.5 font-mono font-bold text-[var(--orbit-text-primary)] sm:px-6 sm:py-4">
                    <div className="flex items-center gap-2">
                      <div className="flex h-5 w-5 items-center justify-center rounded bg-[var(--orbit-accent-primary)]/10">
                        <FileText className="h-3 w-3 text-[var(--orbit-accent-primary)]" />
                      </div>
                      <span>{inv.id}</span>
                    </div>
                  </td>
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[var(--orbit-border-subtle)] bg-[var(--orbit-elevated)]/50 px-4 py-3 text-xs sm:flex-row sm:px-6 sm:py-4">
          <div className="font-medium text-[var(--orbit-text-muted)]">
            Showing {filteredInvoices.length > 0 ? 1 : 0} to {filteredInvoices.length} of {filteredInvoices.length} results
          </div>
      ...
      {/* Slide-over Drawer for Create Invoice */}
      {isDrawerOpen && (
            ...
            <div className="flex-1 space-y-6 overflow-y-auto p-4 md:p-6 dark:border-gray-800">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Calendar className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-[var(--orbit-text-muted)]" />
                  <input
                    type="text"
                    placeholder="Oct 31, 2026"
                    className="h-11 w-full rounded-xl border border-[var(--orbit-border-mid)] bg-[var(--orbit-surface)] pr-4 pl-9 text-base text-[var(--orbit-text-primary)] transition-all placeholder:text-[var(--orbit-text-muted)] focus:border-[var(--orbit-accent-primary)] focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 sm:text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                   aria-label="Oct 31, 2026" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="block font-mono text-xs font-bold tracking-widest text-[var(--orbit-text-muted)] uppercase dark:text-[var(--orbit-text-secondary)]">
              ...
              <div className="space-y-4">
                <div className="flex flex-col items-center justify-center rounded-2xl bg-[var(--orbit-elevated)] p-6 dark:bg-gray-800/50">
                  ...
                  <div className="mt-1 font-mono text-xs font-bold tracking-widest text-[var(--orbit-text-muted)] uppercase">
                    Amount {selectedInvoice.status === 'Paid' ? 'Paid' : selectedInvoice.status}
                  </div>
                </div>
                ...
                <div className="border-t border-[var(--orbit-border-subtle)] pt-4">
                  <div className="mb-2 font-mono text-xs font-bold tracking-widest text-[var(--orbit-text-muted)] uppercase dark:text-gray-400">
                    Line Items
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-[var(--orbit-elevated)] p-4 dark:bg-gray-800/40">
                    <div>
                      <div className="text-sm font-semibold text-[var(--orbit-text-primary)] dark:text-gray-200">
                        Platform Subscription Fees
                      </div>
                      <div className="text-xs text-[var(--orbit-text-muted)]">Qty: 1</div>
                    </div>
                    <div className="font-mono text-sm font-bold text-[var(--orbit-text-primary)] dark:text-gray-100">
                      ${selectedInvoice.amount}
                    </div>
                  </div>
                </div>
