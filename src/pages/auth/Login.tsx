...
export const Login = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Simulate auth - store session token and redirect to dashboard
    setTimeout(() => {
      localStorage.setItem(
        'orbitflow_session',
        JSON.stringify({
          user: { name: 'Demo User', email: form.email || 'marcus@orbitflow.io', role: 'Admin' },
        ...
        >
          ...
          <div className="elite-card rounded-2xl p-6 sm:p-7 text-left">
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              ...
              <div className="space-y-2">
                <div className="ml-1 flex items-center justify-between">
                  <label className="block font-mono text-xs font-semibold tracking-widest text-[var(--orbit-text-muted)] uppercase">
                    Password
                  </label>
                  <Link
                    to="/auth/forgot-password"
                    className="text-xs font-semibold tracking-wider text-[var(--orbit-accent-primary)] uppercase hover:underline"
                  >
                    Forgot?
                  </Link>
                </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <button
                type="button"
                onClick={() => showToast("Social login coming soon — use email & password for now")}
